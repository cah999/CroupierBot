...
class Economic(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if xp >= 500 + 100 * lvl:
            ...
            self.cursor.execute(
                "UPDATE users SET balance = balance + {} WHERE id = {}".format(lvl * 100 + 100, user.id))
            self.conn.commit()
            self.cursor.execute("SELECT lvl FROM users WHERE id = {}".format(user.id))
            lvl = self.cursor.fetchone()[0]
            # await user.send(embed=discord.Embed(
            #     description=f'Поздравляю! Ты получил **{lvl}** уровень!\nТвоя награда: **{lvl * 100} :tickets: **',
            #     color=0xfbff00))
            if lvl == 10:
                embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                      description="**⭐️Мама, кажется, я расту!⭐️**", color=0x66fcff)
                await user.send(embed=embed)
                self.cursor.execute("UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, user.id))
                self.conn.commit()
            elif lvl == 20:
                embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🌟Биг Бой🌟**",
                                      color=0x66fcff)
                await user.send(embed=embed)
                self.cursor.execute("UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, user.id))
                self.conn.commit()
            elif lvl == 30:
                embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**✨Дедуля✨**",
                                      color=0x66fcff)
                await user.send(embed=embed)
                self.cursor.execute("UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, user.id))
                self.conn.commit()
        ...
        if msg == 100:
            embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**✏️На скорую руку✏️**",
                                  color=0x66fcff)
            ...
            embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🖋Ну, я тут настрочил🖋**",
                                  color=0x66fcff)
            ...
            embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**📚Наследник Толстого📚**",
                                  color=0x66fcff)
        ...
        if after.channel:
            if after.mute or after.self_mute:
                ...
                while after.channel and not after.self_mute:
                    ...
                    self.cursor.execute(
                        "UPDATE users SET voice_minutes = voice_minutes + {} WHERE id = {}".format(1, member.id))
                    ...
                    if xp >= 500 + 100 * lvl:
                        ...
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(lvl * 100 + 100, member.id))
                        self.conn.commit()
                        self.cursor.execute("SELECT lvl FROM users WHERE id = {}".format(member.id))
                        lvl = self.cursor.fetchone()[0]
                        ...
                        if lvl == 10:
                            ...
                            self.cursor.execute(
                                "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, member.id))
                            self.conn.commit()
                        elif lvl == 20:
                            embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🌟Биг Бой🌟**",
                                                  color=0x66fcff)
                            ...
                            if ach == 27:
                                ...
                                embed = discord.Embed(title="Ты получил ПОСЛЕДНЕЕ достижение!🥳",
                                                      description="**🏆ЛЕГЕНДА🏆**", color=0x66fcff)
                                embed.add_field(name="Твоя награда",
                                                value="**10000 :tickets:**\nА также эксклюзивная роль ``👑Склоните колено👑``",
                                                inline=False)
                                self.cursor.execute(
                                    "UPDATE users SET balance = balance + {} WHERE id = {}".format(10000, member.id))
                                self.conn.commit()
                                await member.send(embed=embed)
                    self.cursor.execute("SELECT voice_minutes FROM users WHERE id = {}".format(member.id))
                    voice = self.cursor.fetchone()[0]
                    if voice == 60 * 100:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🥉Орунькаю🥉**",
                                              color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                        await member.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, member.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, member.id))
                        self.conn.commit()

                    elif voice == 60 * 500:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🥈Орирую🥈**",
                                              color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**2000 :tickets:**", inline=False)
                        await member.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(2000, member.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, member.id))
                        self.conn.commit()
                        ...
                        embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                              description="**🥇У меня нет рта, но я должен кричать🥇**", color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**3000 :tickets:**", inline=False)
                        await member.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(3000, member.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, member.id))
                        self.conn.commit()
                        self.cursor.execute("SELECT achivements FROM users WHERE id = {}".format(member.id))
    ...
    # Info
    @commands.command(aliases=['info', 'INFO', 'Info', 'balance'],
                      usage='!info / !info <@user> (при достижении 15 уровня)')
    async def __info(self, ctx, member: discord.Member = None):
        ...
        if member is None:
            ...
            embed.add_field(name="Дата захода на сервер", value=f'**{ctx.author.joined_at.strftime("%m/%d/%Y")}** 📅',
                            inline=False)
            ...
            embed.add_field(name="Количество сообщений", value=f'**{self.cursor.fetchone()[0]} :e_mail:**',
                            inline=False)
            self.cursor.execute("SELECT voice_minutes FROM users WHERE id = {}".format(ctx.author.id))
            a = self.cursor.fetchone()[0]
            if a // 60 < 10:
                if a % 60 < 10:
                    embed.add_field(name="Время проведённое в войсе", value=f'**0{a // 60}:0{a % 60} 🎤 **',
                                    inline=False)
                else:
                    embed.add_field(name="Время проведённое в войсе", value=f'**0{a // 60}:{a % 60} 🎤 **',
                                    inline=False)
            else:
                if a % 60 < 10:
                    embed.add_field(name="Время проведённое в войсе", value=f'**{a // 60}:0{a % 60} 🎤 **',
                                    inline=False)
                else:
            ...
            if self.cursor.fetchone()[0] < 15:
                await ctx.author.send(embed=discord.Embed(
                    description='Для просмотра профелей других людей вам нужно иметь **15 уровень**'), delete_after=60)
            else:
                ...
                embed.add_field(name="Дата захода на сервер", value=f'**{member.joined_at.strftime("%m/%d/%Y")} 📅**',
                                inline=False)
                ...
                if a // 60 < 10:
                    ...
                    if a % 60 < 10:
                        ...
                        embed.add_field(name="Время проведённое в войсе", value=f'**{a // 60}:{a % 60} 🎤 **',
                                        inline=False)
                self.cursor.execute("SELECT warns FROM users WHERE id = {}".format(member.id))
                embed.add_field(name="Количество варнов", value=f'**{self.cursor.fetchone()[0]} :no_entry:**',
                                inline=False)
                await ctx.author.send(embed=embed, delete_after=60 * 5)
    ...
    async def __daily(self, ctx):
        ...
        self.conn.commit()
        embed = discord.Embed(
            description=f':white_check_mark: **{ctx.author.name}** ты получил **{amount}** :tickets:!', color=0xe59eff)
    ...
    async def daily_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.CommandOnCooldown):
            await ctx.author.send(embed=discord.Embed(
                description=f':no_entry: {ctx.author.name}, ты сегодня уже брал дневной бонус\nПопробуй снова через: **{convert(n)}**!',
                color=0xFFA500), delete_after=60)
    ...
    #             self.conn.commit()
    #             await member.send(embed=discord.Embed(
    #                 description=f'Твой баланс пополнен на **{amount}** :tickets: !\n\n*С любовью,* **{ctx.author.name}** *<3*'),
    #                 delete_after=60)
    ...
    #                 self.conn.commit()
    #                 await member.send(embed=discord.Embed(
    #                     description=f'С **вашего** баланса сняты **все** средства!\n\n*С любовью,* **{ctx.author.name}** *<3*'),
    #                     delete_after=60)
    #             else:
    #                 await member.send(embed=discord.Embed(
    #                     description=f'С **вашего** баланса снято **{amount}** :tickets: !\n\n*С любовью,* **{ctx.author.name}** *<3*'),
    #                     delete_after=60)
    ...
    #         else:
    #             self.cursor.execute(
    #                 "UPDATE users SET balance = balance - {} WHERE id = {}".format(amount, ctx.author.id))
    #             self.cursor.execute("UPDATE users SET balance = balance + {} WHERE id = {}".format(amount, member.id))
    #             self.conn.commit()
    #             await member.send(
    #                 embed=discord.Embed(description=f'**{ctx.author.name}** перевёл вам **{amount}** :tickets: !'),
    #                 delete_after=60)
    ...
    async def __slot(self, ctx, amount: int = None):
        ...
        if amount is None:
            ...
            await ctx.send(embed=discord.Embed(description=f'**{ctx.author.name}**, укажите сумму больше 0 :tickets:'),
                           delete_after=5)
            ctx.command.reset_cooldown(ctx)
        else:
            if ubalance < amount:
                await ctx.send(
                    embed=discord.Embed(description=f"**{ctx.author}**, у вас недостаточно средств для игры!"),
                    delete_after=5)
                ctx.command.reset_cooldown(ctx)
            else:
                self.cursor.execute(
                ...
                for i in range(n):
                    new_embed = discord.Embed(
                        description=f'| {random.choice(emojis)} | {random.choice(emojis)} | {random.choice(emojis)} |')
                ...
                if (a == b == c):
                    self.cursor.execute(
                        "UPDATE users SET balance = balance + {} WHERE id = {}".format(amount * 3, ctx.author.id))
                    self.conn.commit()
                    self.cursor.execute("SELECT balance FROM users WHERE id = {}".format(ctx.author.id))
                    bal = self.cursor.fetchone()[0]
                    embed1 = discord.Embed(
                        description=f'{slotmachine} 3 в ряд, ты победил! 🎉\n Ваш баланс: **{bal}** :tickets:')
                    await message.edit(embed=embed1)
                    self.cursor.execute(
                        "UPDATE users SET slots_wins = slots_wins + {} WHERE id = {}".format(1, ctx.author.id))
                    self.conn.commit()
                    self.cursor.execute("SELECT slots_wins FROM users WHERE id = {}".format(ctx.author.id))
                    a = self.cursor.fetchone()[0]
                    if a == 100:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**♟Опа стрик♟**",
                                              color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                        await ctx.author.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, ctx.author.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, ctx.author.id))
                        self.conn.commit()
                    elif a == 250:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                              description="**💰Мама, накрывай стол💰 **", color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**2500 :tickets:**", inline=False)
                        await ctx.author.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(2500, ctx.author.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, ctx.author.id))
                        self.conn.commit()
                    elif a == 500:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                              description="**💎ЛЕГЧАЙШАЯ ДЛЯ ВЕЛИЧАЙШЕГО💎**", color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**5000 :tickets:**", inline=False)
                        await ctx.author.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(5000, ctx.author.id))
                        self.cursor.execute(
                            "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, ctx.author.id))
                        self.conn.commit()
                    ...
                    self.cursor.execute(
                        "UPDATE users SET balance = balance + {} WHERE id = {}".format(amount * 2, ctx.author.id))
                    self.conn.commit()
                    self.cursor.execute("SELECT balance FROM users WHERE id = {}".format(ctx.author.id))
                    bal = self.cursor.fetchone()[0]
                    embed2 = discord.Embed(
                        description=f'{slotmachine} 2 в ряд, ты победил! 🎉 \n Ваш баланс: **{bal}** :tickets:')
                    ...
                    embed3 = discord.Embed(
                        description=f'{slotmachine} Нет совпадений, ты проиграл 😢\n Ваш баланс: **{bal}** :tickets:')
    ...
    async def slot_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.CommandOnCooldown):
            await ctx.send(embed=discord.Embed(
                description=f':no_entry: {ctx.author.mention}, не так быстро. \nКрутить слоты можно раз в 10 секунд! \nПопробуй снова через: {round(error.retry_after)} секунд!',
                color=0xFFA500), delete_after=5)
        ...
        if member is None:
            ...
            if amount is None:
                ...
                if ubal < amount:
                    ...
                    await ctx.send(f"**{ctx.author.name}**, у {member.name} недостаточно средств для дуэли!",
                                   delete_after=5)
                    ctx.command.reset_cooldown(ctx)
                else:
                    message = await ctx.send(
                        f'**{ctx.author.name}** бросил дуэль **{member.name}**\nЖдём подтверждения от {member.mention} \n**{member.name}**, нажми на реакцию, чтобы принять участие в дуэли',
                        delete_after=30)
                    ...
                    try:
                        ...
                        if ubal < amount:
                            await ctx.send(
                                f"**{ctx.author}**, у вас недостаточно средств для дуэли!\n Не надо пытаться меня переиграть!",
                                delete_after=5)
                            ctx.command.reset_cooldown(ctx)
                        elif mbal < amount:
                            await ctx.send(
                                f"**{ctx.author}**, у {member.name} недостаточно средств для дуэли!\nНе нужно пытаться мне переиграть!",
                                delete_after=5)
                            ctx.command.reset_cooldown(ctx)
                        ...
                        if n == 1:
                            self.cursor.execute(
                                "UPDATE users SET balance = balance + {} WHERE id = {}".format(amount, ctx.author.id))
                            self.cursor.execute(
                                "UPDATE users SET balance = balance - {} WHERE id = {}".format(amount, member.id))
                            self.conn.commit()
                            self.cursor.execute("SELECT balance FROM users WHERE id = {}".format(ctx.author.id))
                            ubal = self.cursor.fetchone()[0]
                            self.cursor.execute("SELECT balance FROM users WHERE id = {}".format(member.id))
                            mbal = self.cursor.fetchone()[0]
                            await ctx.send(
                                f'**{ctx.author}** победил! Ему начислено **{amount} :tickets:** \nБаланс {ctx.author.name} составляет __{ubal}__:tickets:\nБаланс **{member.name}** составляет __{mbal}__:tickets:',
                                delete_after=15)
                            self.cursor.execute(
                                "UPDATE users SET duel_wins = duel_wins + {} WHERE id = {}".format(1, ctx.author.id))
                            self.cursor.execute(
                                "UPDATE users SET duel_loses = duel_loses + {} WHERE id = {}".format(1, member.id))
                            self.conn.commit()
                            ...
                            if a == 100:
                                embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                                      description="**🔫Дантес🔫**", color=0x66fcff)
                                embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                                await ctx.author.send(embed=embed)
                                self.cursor.execute(
                                    "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, ctx.author.id))
                                self.cursor.execute(
                                    "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1,
                                                                                                           ctx.author.id))
                                self.conn.commit()
                                self.cursor.execute("SELECT achivements FROM users WHERE id = {}".format(ctx.author.id))
                                b = self.cursor.fetchone()[0]
                                if b == 27:
                                    ...
                                    embed.add_field(name="Твоя награда",
                                                    ...
                                                    inline=False)
                                    self.cursor.execute(
                                        "UPDATE users SET balance = balance + {} WHERE id = {}".format(10000,
                                                                                                       ctx.author.id))
                                    self.conn.commit()
                                    await ctx.author.send(embed=embed)
                            elif c == 100:
                                embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                                      description="**🩸Пушкин🩸 **", color=0x66fcff)
                                embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                                await member.send(embed=embed)
                                self.cursor.execute(
                                    "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, member.id))
                            ...
                            mbal = self.cursor.fetchone()[0]
                            await ctx.send(
                                f'**{member}** победил! Ему начислено **{amount} :tickets:**\nБаланс {member.name} составляет __{mbal}__:tickets:\nБаланс **{ctx.author.name}** составляет __{ubal}__:tickets:',
                                delete_after=15)
                            self.cursor.execute(
                                "UPDATE users SET duel_wins = duel_wins + {} WHERE id = {}".format(1, member.id))
                            self.cursor.execute(
                                "UPDATE users SET duel_loses = duel_loses + {} WHERE id = {}".format(1, ctx.author.id))
                            self.conn.commit()
    ...
    async def duel_error(self, ctx, error):
        if isinstance(error, commands.errors.CommandOnCooldown):
            await ctx.send(embed=discord.Embed(
                description=f':no_entry: {ctx.author.mention}, тихо-тихо, не так быстро. \nНужно перезарядить револьвер \nПопробуй снова через: {round(error.retry_after)} секунд!',
                color=0xFFA500))
    ...
    async def __crime(self, ctx):
        ...
        if lvl < 10:
            await ctx.send(embed=discord.Embed(
                description=f'**{ctx.author.name}**, для доступа к ограблениям вам нужен **10 уровень**',
                color=discord.Color.red()), delete_after=10)
        elif bal < 0:
            await ctx.send(embed=discord.Embed(description=f'**{ctx.author.name}**, у вас отрицательный баланс!**',
                                               color=discord.Color.red()), delete_after=10)

        else:
            amount = random.randint(0, 5001)
            message = await ctx.send(embed=discord.Embed(
                description=f'**{ctx.author.name}**, нам удалось обнаружить, что в сейфе находится **{amount} :tickets:**\nВы уверены, что хотите продолжить?',
            ...
            if str(reaction.emoji) == '✅':
                ...
                if n == 0:
                    ...
                    e = discord.Embed(
                        description=f'**{ctx.author.name}**, поздравляю!\nТебе уже перечислили **{amount}** :tickets: на твой счёт')
                    await message.edit(embed=e)
                    self.cursor.execute(
                        "UPDATE users SET balance = balance + {} WHERE id = {}".format(amount, ctx.author.id))
                    self.conn.commit()
                    self.cursor.execute(
                        "UPDATE users SET crime_win = crime_win + {} WHERE id = {}".format(1, ctx.author.id))
                    self.conn.commit()
                    ...
                    if a == 100:
                        embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                              description="**💎День зарплаты💎**", color=0x66fcff)
                        embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                        await ctx.author.send(embed=embed)
                        self.cursor.execute(
                            "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, ctx.author.id))
                    ...
                    if str(reaction.emoji) == '🎭':
                        ...
                        if escape == 0:
                            ...
                            self.cursor.execute(
                                "UPDATE users SET crime_lose = crime_lose + {} WHERE id = {}".format(1, ctx.author.id))
                            self.conn.commit()
                            self.cursor.execute("SELECT crime_lose FROM users WHERE id = {}".format(ctx.author.id))
                            b = self.cursor.fetchone()[0]
                            if b == 50:
                                embed = discord.Embed(title="Ты получил новое достижение!🥳",
                                                      description="**🙏Матвей🙏**", color=0x66fcff)
                                embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
                                await ctx.author.send(embed=embed)
                                self.cursor.execute(
                                    "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, ctx.author.id))
                            ...
                            emb1 = discord.Embed(
                                description=f'К сожалению, ты выбрал неверный способ побега. Теперь тебе нужно заплатить штраф в размере **{amount * 1.5}:tickets:** ')
                            await message.edit(embed=emb1)
                            self.cursor.execute(
                                "UPDATE users SET balance = balance - {} WHERE id = {}".format(amount * 1.5,
                                                                                               ctx.author.id))
                            self.conn.commit()
                            await asyncio.sleep(10)
                            await message.delete()
    ...
    async def crime_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.CommandOnCooldown):
            await ctx.author.send(embed=discord.Embed(
                description=f':no_entry: {ctx.author.name}, тихо-тихо, подожди пока всё утихнет\nПопробуй снова через: **{convert(n)}**!',
                color=0xFFA500), delete_after=60)
    ...
    #                         if n > 50:
    #                             self.cursor.execute(
    #                                 "UPDATE users SET balance = balance + {} WHERE id = {}".format(amount * 2,
    #                                                                                                ctx.author.id))
    #                             self.conn.commit()
    #                             self.cursor.execute("SELECT balance FROM users WHERE id = {}".format(ctx.author.id))
    #                             ubal = self.cursor.fetchone()[0]
    #                             await ctx.send(embed=discord.Embed(
    #                                 description=f'{ctx.author.name} **успех** ! \nВаш баланс: __{ubal}__ :tickets:'),
    #                                 delete_after=10)
    #                             self.cursor.execute(
    #                                 "UPDATE users SET nvuti_wins = nvuti_wins + {} WHERE id = {}".format(1,
    #                                                                                                      ctx.author.id))
    #                             self.conn.commit()
    ...
    #                             if a == 100:
    #                                 embed = discord.Embed(title="Ты получил новое достижение!🥳",
    #                                                       description="**💵Stonks!💵**", color=0x66fcff)
    #                                 embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
    #                                 await ctx.author.send(embed=embed)
    #                                 self.cursor.execute(
    #                                     "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000,
    #                                                                                                    ctx.author.id))
    #                                 self.cursor.execute(
    #                                     "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1,
    #                                                                                                            ctx.author.id))
    #                                 self.conn.commit()
    #                                 self.cursor.execute(
    #                                     "SELECT achivements FROM users WHERE id = {}".format(ctx.author.id))
    #                                 ach = self.cursor.fetchone()[0]
    #                                 if ach == 27:
    #                                     role = discord.utils.get(ctx.author.guild.roles, name="👑Склоните колено👑")
    #                                     await ctx.author.add_roles(role)
    ...
    #                             ubal = self.cursor.fetchone()[0]
    #                             await ctx.send(embed=discord.Embed(
    #                                 description=f'{ctx.author.name} в этот раз не повезло!\nВаш баланс __{ubal}__ :tickets:'),
    #                                 delete_after=10)
    #                     elif result == '-':
    #                         if n < 50:
    #                             self.cursor.execute(
    ...
    #                     else:
    #                         await ctx.send(
    #                             f'{ctx.author.name} правильное использование команды: ``!nvuti <amount> <+/->``',
    #                             delete_after=5)
    #                         ctx.command.reset_cooldown(ctx)
    #
    # @__nvuti.error
    ...
    #                 ubal = self.cursor.fetchone()[0]
    #                 await ctx.send(embed=discord.Embed(
    #                     description=f'**{ctx.author.name}**, поздравляю с победой!\nВаш баланс __{ubal}__ :tickets:'),
    #                     delete_after=10)
    #                 self.cursor.execute(
    #                     "UPDATE users SET coinflip_wins = coinflip_wins + {} WHERE id = {}".format(1, ctx.author.id))
    #                 self.conn.commit()
    #                 self.cursor.execute("SELECT coinflip_wins FROM users WHERE id = {}".format(ctx.author.id))
    #                 a = self.cursor.fetchone()[0]
    #                 if a == 100:
    #                     embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**💵Stonks!💵**",
    #                                           color=0x66fcff)
    #                     embed.add_field(name="Твоя награда", value="**1000 :tickets:**", inline=False)
    #                     await ctx.author.send(embed=embed)
    #                     self.cursor.execute(
    ...
    #                 ubal = self.cursor.fetchone()[0]
    #                 await ctx.send(embed=discord.Embed(
    #                     description=f'**{ctx.author.name}**, вы проиграли!\nВаш балас __{ubal}__ :tickets:'),
    #                     delete_after=10)
    #
    # @__coinflip.error
    # async def crime_error(self, ctx, error):
    ...
    #                     self.cursor.execute(
    #                         "UPDATE users1 SET server_id = {} WHERE id = {}".format(guild.id, member.id))
    #                     print(guild.id)
