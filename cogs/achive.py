...
class achive(commands.Cog):
    ...
    async def on_message(self, message):
        if message.content.startswith('.play'):
            self.cursor.execute(
                "UPDATE users SET music_tracks = music_tracks + {} WHERE id = {}".format(1, message.author.id))
            self.conn.commit()
            ...
            if a == 200:
                ...
                embed = discord.Embed(title="Ты получил новое достижение!🥳", description="**🎧Диджей🎧**",
                                      color=0x66fcff)
                embed.add_field(name="Твоя награда",
                                value="**1000 :tickets:**\nА так же эксклюзивная роль ``📀Ебан💿``", inline=False)
                await message.author.send(embed=embed)
                self.cursor.execute(
                    "UPDATE users SET balance = balance + {} WHERE id = {}".format(1000, message.author.id))
                self.cursor.execute(
                    "UPDATE users SET achivements = achivements + {} WHERE id = {}".format(1, message.author.id))
                self.conn.commit()
                await message.author.send(embed=embed)
                ...
                if ach == 27:
                    ...
                    embed.add_field(name="Твоя награда",
                                    ...
                                    inline=False)
                    self.cursor.execute(
                        "UPDATE users SET balance = balance + {} WHERE id = {}".format(10000, message.author.id))
                    self.conn.commit()
