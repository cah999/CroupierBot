...
answer5 = ['Вы не можете замутить пользователя с такой же ролью!', 'Ваши роли одинаковы, я не могу так сделать!',
           'Вы не можете замутить такого же модератора как и вы!']
answer4 = ['Это невозможно сделать, так как выгнать меня может только основатель сервера!',
           'Это может сделать только основатель сервера', 'Так сделать невозможно!',
           'Увы, меня нельзя так остранить...']
answer3 = ['У вас не хватает прав!', 'Его роль стоит выше вашей!', 'Это нельзя сделать!',
           'Ваша роль менее значима, чем этого пользователя!']
answer2 = ['Ты быканул на основателя сервера, или мне показалось?', 'Что он такого плохого тебе сделал?',
           'При всём уважении к тебе я так не могу сделать!', 'Ах если бы я так мог...', 'Я не буду этого делать!',
           'Сорян, но не в моих это силах!']
answer = ['Самоубийство не приведёт ни к чему хорошему!', 'Напомню: суицид - не выход!',
          'Увы, я не могу этого сделать!', 'Самоубийство - не выход!', 'Не надо к себе так относиться!',
          'Я не сделаю этого!', 'Я не буду это делать!', 'Я не выполню это действие', 'Не заставляй меня это сделать!']
...
class mod(commands.Cog):
            ...
            if message.content.startswith('!'):
                ...
                if message.author == self.client.user:
                    ...
                    await message.author.send(embed=discord.Embed(
                        description=f'В канале ``{channel.name}`` можно писать **только** команды для **бота**!'),
                        delete_after=30)
    ...
    async def clear(self, ctx, amount=100):
        ...
        if (amount == 0) or (amount < 0):
            await ctx.send(embed=discord.Embed(description=f':x: Введите количество сообщений для удаления!'),
                           delete_after=3)
        else:
    ...
    async def kick(self, ctx, member: discord.Member, *, reason=None):
        ...
        if member.id == 312795489743405058 or member.id == 475975938577006595 or member.id == 340368453976457216 or member.id == 483866841148686337 or member.id == 715914571839176786 or member.id == 601309336454823936:
            ...
            await member.send(
                embed=discord.Embed(description=f"Вы были кикнуты с сервера!\nПричина {reason}", color=0x5abaa7))
    ...
    async def ban(self, ctx, member: discord.Member, *, reason=None):
        ...
        if member.id == 312795489743405058 or member.id == 475975938577006595 or member.id == 340368453976457216 or member.id == 483866841148686337 or member.id == 715914571839176786 or member.id == 601309336454823936:
            ...
            await member.send(embed=discord.Embed(
                description=f"Вы были заблокированы на сервере!\nПричина **{reason}**\nДля разблокировки обращайтесь к {ctx.author.mention}",
                color=0x5abaa7))
    ...
    async def clearwarns(self, ctx, member: discord.Member):
        ...
        self.conn.commit()
        await ctx.send(embed=discord.Embed(description=f'С пользователя **{member.name}** сняты все варны!'),
                       delete_after=6)
    ...
    #     await ctx.message.delete()
    #     if member is None:
    #         embed = discord.Embed(title="Ошибка", description="Укажите пользователя `!report <member> <reason>`",
    #                               color=discord.Color.red())
    #         await ctx.send(embed=embed, delete_after=10)
    #         ctx.command.reset_cooldown(ctx)
    #     elif reason is None:
    #         embed = discord.Embed(title="Ошибка", description="Укажите причину жалобы `!report <member> <reason>`",
    #                               color=discord.Color.red())
    #         await ctx.send(embed=embed, delete_after=10)
    #         ctx.command.reset_cooldown(ctx)
    #     elif member == ctx.author:
    #         embed = discord.Embed(title="Ошибка", description="Вы не можете отправить жалобу на себя",
    #                               color=discord.Color.red())
    #         await ctx.send(embed=embed, delete_after=10)
    #         ctx.command.reset_cooldown(ctx)
    #     else:
    ...
    #                                       description=f'Жалоба была успешно отправлена в канал для жалоб!',
    #                                       color=discord.Color.green())
    #                 await ctx.send(embed=embed, delete_after=10)
    #                 embed2 = discord.Embed(
    #                     title='Новая Жалоба!", description=f"**Отправитель:** {ctx.author.mention}\n\n**Нарушитель:** {member.mention}\n\n**Причина:** {reason}',
    #                     color=discord.Color.green())
    ...
    #                         description='Жалоба от пользователя {ctx.author.mention успешно принята!'))
    #                 else:
    #                     await message.delete()
    #                     await channel.send(
    #                         embed=discord.Embed(description=f'Жалоба от пользователя {ctx.author.mention} отклонена!'),
    #                         delete_after=15)
    #
    #                 break
    #         else:
    #             channel = ctx.guild.get_channel(779274823511310376)
    #             embed = discord.Embed(title=f"Жалоба", timestamp=datetime.datetime.now(datetime.timezone.utc),
    #                                   description=f':white_check_mark: Жалоба была успешно отправлена в канал для жалоб!',
    #                                   color=discord.Color.green())
    #             await ctx.send(embed=embed, delete_after=10)
    #             embed2 = discord.Embed(title="Новая жалоба", timestamp=datetime.datetime.now(datetime.timezone.utc),
    #                                    color=0xff0000)
    ...
    #                 await message.delete()
    #                 await channel.send(
    #                     embed=discord.Embed(description=f'Жалоба от {ctx.author.mention} успешно принята!'),
    #                     delete_after=15)
    #             else:
    #                 await message.delete()
    #                 await channel.send(embed=discord.Embed(description=f'Жалоба от {ctx.author.mention} отклонена!'),
    #                                    delete_after=15)
    #
    ...
    #             for i in ctx.message.attachments:
    #                 channel = ctx.guild.get_channel(779274823511310376)
    #                 embed = discord.Embed(title="Баг!",
    #                                       description=f":white_check_mark: Баг был успешно отправлен в канал для багов!",
    #                                       color=discord.Color.green())
    #                 await ctx.send(embed=embed, delete_after=10)
    #                 embed2 = discord.Embed(title="Новый баг!", timestamp=datetime.datetime.now(datetime.timezone.utc),
    #                                        color=0xff0000)
    #                 embed2.add_field(name="Отправитель", value=f"{ctx.author.mention}", inline=False)
    ...
    #                 reaction, user = await self.client.wait_for('reaction_add', check=check)
    #                 await msg.delete()
    #                 await channel.send(embed=discord.Embed(description=f'Баг от {ctx.author.mention} обработан!',
    #                                                        color=discord.Color.green()), delte_after=15)
    #                 break
    #         else:
    #             channel = ctx.guild.get_channel(779274823511310376)
    #             embed = discord.Embed(title="Баг", timestamp=datetime.datetime.now(datetime.timezone.utc),
    #                                   description=f":white_check_mark: Баг был успешно отправлен в канал для багов!",
    #                                   color=0xfbff00)
    #             await ctx.send(embed=embed, delete_after=10)
    #             embed2 = discord.Embed(title="Новый баг!", timestamp=datetime.datetime.now(datetime.timezone.utc),
    ...
    #             await msg.delete()
    #             await channel.send(embed=discord.Embed(description=f'Баг от {ctx.author.mention} обработан!',
    #                                                    color=discord.Color.green()), delete_after=15)
    #
    # @__report.error
    ...
    #             description=f':no_entry: {ctx.author.mention}, не так быстро!. \nОтправлять жалобы можно раз в 10 секунд!',
    #             color=0xFFA500), delete_after=15)
